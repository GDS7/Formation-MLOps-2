...
def monitor(latest_predictions: pd.DataFrame) -> pd.DataFrame:
    ...
    ptm = latest_predictions["predictions_time"].mean()
    p = latest_predictions["predictions"].mean()
    monitoring_df = pd.DataFrame([[ptm, p]], columns=["predictions_time","predictions"])
